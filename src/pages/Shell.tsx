...
export default function Shell({ path, children }: { path: string; children: ReactNode }) {
  ...
  const active = path.startsWith("/classes") || path.startsWith("/class/")
    ? "classes"
  ...
  return (
    <div className="layout">
      ...
      <aside className={`sidebar ${open ? "open" : ""}`}>
        ...
        <button className={`nav-btn ${active === "classes" ? "active" : ""}`} onClick={() => go("/classes")}>My Classes</button>
        <button className={`nav-btn ${active === "reports" ? "active" : ""}`} onClick={() => go("/reports")}>Reports</button>
        ...
        <div className="side-foot">
          ...
          <div className="who">{current?.institution}</div>
        </div>
      </aside>
      <div className="main">
