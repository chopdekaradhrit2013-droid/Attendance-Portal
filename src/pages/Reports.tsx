...
export default function Reports() {
  ...
  if (current?.role === "student") return <div className="panel">Reports are only available to professors.</div>;
  ...
  return (
    <>
      <div className="topbar"><div><h1>Reports</h1><p>Export a class register whenever you need it.</p></div></div>
      <section className="panel">
        <div className="row" style={{ marginBottom: 16 }}>
          <select className="search" value={classId} onChange={(e) => setClassId(e.target.value)}>
            {classes.length === 0 && <option value="">No classes</option>}
            {classes.map((c) => <option key={c.id} value={c.id}>{c.subject} · {c.name} {c.division}</option>)}
          </select>
          <input className="search" placeholder="Search students" value={q} onChange={(e) => setQ(e.target.value)} />
          <label className="row" style={{ fontSize: 14 }}><input type="checkbox" checked={onlyLow} onChange={(e) => setOnlyLow(e.target.checked)} /> Below 75%</label>
          <button className="btn" disabled={!cls} onClick={() => {
            ...
            downloadCsv(`${cls.subject.replace(/\s+/g, "-")}-attendance.csv`, [
              ...
              ...rows.map((s) => [s.student.rollNo, s.student.fullName, s.student.email, String(s.present), String(s.absent), String(s.total), s.total ? String(Math.round(s.percentage)) : "", s.total === 0 ? "No data" : s.percentage < 75 ? "Warning" : "Good"]),
            ]);
          }}>Export CSV</button>
        </div>
        {stats && <div className="row" style={{ marginBottom: 12 }}>
